...
class Combatants(models.Model):
    ...
    rank = models.CharField(max_length=70, verbose_name="Звание", blank=True)
    battalion = models.CharField(max_length=100, verbose_name="Батальон", blank=True)
    issue_year = models.IntegerField(verbose_name="Год выпуска", blank=True)
    date_of_birth = models.DateField(verbose_name="Дата рождения", blank=True)
    date_of_death = models.DateField(verbose_name="Дата смерти", blank=True)
    place_of_birth = models.ForeignKey(Places, verbose_name="Место рождения", blank=True, on_delete=models.CASCADE)
